...
from crystal_diffusers.validation.mace.block import (
...
class MACE(torch.nn.Module):
    def __init__(
        ...
        self.dropouts = torch.nn.ModuleList(
            [torch.nn.Dropout(p=0.3) for i in range(len(self.products))]
        )

        self.readout = o3.Linear(hidden_irreps_out, "1x0e")
    ...
    def forward(
        ...
        node_attrs = F.one_hot(
            data.atomic_numbers.long().squeeze(-1),
        ...
        # Interactions
        for interaction, product, dropout in zip(
            self.interactions, self.products, self.dropouts
        ):
