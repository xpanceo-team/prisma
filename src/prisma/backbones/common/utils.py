...
import prisma.backbones
from prisma.backbones.common.registry import registry
...
def new_trainer_context(*, config: dict[str, Any]):
    from prisma.backbones.common import distutils, gp_utils
    from prisma.backbones.common.registry import registry

    if TYPE_CHECKING:
        from prisma.backbones.tasks.task import BaseTask
        from prisma.backbones.trainers import BaseTrainer
...
def _resolve_scale_factor_submodule(model: nn.Module, name: str):
    from prisma.backbones.modules.scaling.scale_factor import ScaleFactor
...
def get_commit_hash() -> str:
    core_hash = get_commit_hash_for_repo(prisma.backbones.__path__[0])
